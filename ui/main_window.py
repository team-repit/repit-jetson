...
# 운동 분석 모듈들은 필요할 때 동적으로 import
# import squat_real_tts  # 제거
# import lunge_realtime  # 제거  
# import plank  # 제거
        ...
        try:
            ...
            if self.exercise_type == "squat":
                ...
                try:
                    ...
                    if hasattr(squat_module, 'run_squat_analysis'):
                    ...
                    elif hasattr(squat_module, 'main'):
                        # main 함수만 있는 경우 - 임시로 실행
                        self.status_updated.emit("main 함수로 분석 실행...")
                        squat_module.main()
                        # 결과 파일을 찾아서 반환
                        import os, glob, time
                        ...
                        if os.path.exists(output_dir):
                            video_files = glob.glob(os.path.join(output_dir, 'squat_*.mp4'))
                            report_files = glob.glob(os.path.join(output_dir, 'squat_*.txt'))
                            if video_files and report_files:
                                ...
                                self.error_occurred.emit("결과 파일을 찾을 수 없습니다.")
                                return
                        else:
                            self.error_occurred.emit("출력 디렉토리를 찾을 수 없습니다.")
                            return
                    else:
                        self.error_occurred.emit("분석 함수를 찾을 수 없습니다.")
                        return
                ...
                self.error_occurred.emit("런지 분석은 아직 준비 중입니다.")
                return
