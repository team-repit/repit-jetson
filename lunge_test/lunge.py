...
# [개선 2: 새로운 MediaPipe Tasks API 임포트]
from mediapipe.tasks import python as mp_tasks
from mediapipe.tasks.python import vision
from dataclasses import dataclass # 래퍼 클래스용
...
# MediaPipe Pose 모델 초기화 (Legacy) - POSE_CONNECTIONS 때문에 유지
mp_pose = mp.solutions.pose
# [개선 2: Legacy pose 객체는 더 이상 사용하지 않음]
# pose = mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) 
mp_drawing = mp.solutions.drawing_utils

# [개선 2: 레거시 mp_drawing과 새 API 호환을 위한 래퍼 클래스]
# mp_drawing.draw_landmarks가 예전 (legacy) 결과 형식을 기대하므로, 
# 새 API의 결과를 예전 형식처럼 보이도록 감싸줍니다.
@dataclass
class LegacyPoseLandmarks:
    landmark: List[mp_tasks.python.components.containers.landmark.NormalizedLandmark]
...
class UniversalTTS(QObject if QT_AVAILABLE else object):
    ...
    def __init__(self):
        ...
        # [개선 1: 피드백 문구를 더 자연스럽고 친근하게 수정]
        self.feedback_messages = {
            "좌우 흔들림": "몸이 좌우로 흔들려요. 배에 힘을 주고 중심을 꽉 잡아보세요!",
            "무릎 모임": "무릎이 안으로 모이고 있어요. 무릎이 발끝과 같은 방향을 보도록 신경 써주세요.",
            "과도한 무릎 전진": "무릎이 발끝보다 너무 앞으로 나갔네요. 엉덩이를 뒤로 살짝 빼면서 무릎을 조절해 보세요.",
            "상체 숙여짐": "상체가 앞으로 숙여지고 있어요. 가슴을 활짝 펴고 허리를 세워주세요.",
            "부족한 깊이": "조금만 더 깊게 앉아볼까요? 뒷무릎이 바닥에 닿기 직전까지 내려가 보세요.",
            "좁은 스탠스": "보폭이 너무 좁아요. 발을 어깨너비 정도로 벌려서 안정적으로 서주세요.",
            "앞발목 가동성 부족": "앞쪽 발목을 조금 더 굽혀볼까요? 발뒤꿈치가 뜨지 않게 주의하세요."
        }
    
    # ... (UniversalTTS의 나머지 메서드는 기존과 동일) ...
...
# ... (ERROR_CRITERIA_MAP 및 save_report 함수는 기존과 동일) ...
ERROR_CRITERIA_MAP = {
...
def run_lunge_analysis(duration_seconds=120, stop_callback=None, frame_callback=None, is_gui_mode=False):
    ...
    landmarker = None # Landmarker 객체를 담을 변수
    ...
    try:
        ...
        # [개선 2: MediaPipe Pose Landmarker (최신 API) 초기화]
        print("MediaPipe Pose Landmarker 초기화 중...")
        MODEL_PATH = 'pose_landmarker_full.task' # 또는 'pose_landmarker_lite.task'
        if not os.path.exists(MODEL_PATH):
            print(f"오류: {MODEL_PATH} 모델 파일을 찾을 수 없습니다.")
            print("wget -q -O pose_landmarker_full.task https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_full/float16/1/pose_landmarker_full.task")
            print("위 명령어로 모델을 다운로드하세요.")
            cap.release()
            out.release()
            return None, None
    ...
    while cap.isOpened():
        ...
        try:
            # [개선 2: 랜드마크 추출 방식 변경]
            # results.pose_landmarks는 이제 List[List[NormalizedLandmark]]
            # 0번 인덱스 = 첫 번째로 감지된 사람
            landmarks_list = results.pose_landmarks[0]
            h, w, _ = image.shape
            
            # [개선 2: 기존 로직과 호환]
            # 기존 코드가 mp_pose.PoseLandmark.LEFT_SHOULDER.value (정수 11) 등을
            # 인덱스로 사용하므로, 새 API의 landmarks_list (리스트)와 완벽히 호환됨.
            lm_data = {
                'left_shoulder': [landmarks_list[mp_pose.PoseLandmark.LEFT_SHOULDER.value].x * w, landmarks_list[mp_pose.PoseLandmark.LEFT_SHOULDER.value].y * h],
                'left_hip': [landmarks_list[mp_pose.PoseLandmark.LEFT_HIP.value].x * w, landmarks_list[mp_pose.PoseLandmark.LEFT_HIP.value].y * h],
                'left_knee': [landmarks_list[mp_pose.PoseLandmark.LEFT_KNEE.value].x * w, landmarks_list[mp_pose.PoseLandmark.LEFT_KNEE.value].y * h],
                'left_ankle': [landmarks_list[mp_pose.PoseLandmark.LEFT_ANKLE.value].x * w, landmarks_list[mp_pose.PoseLandmark.LEFT_ANKLE.value].y * h],
                'left_foot_index': [landmarks_list[mp_pose.PoseLandmark.LEFT_FOOT_INDEX.value].x * w, landmarks_list[mp_pose.PoseLandmark.LEFT_FOOT_INDEX.value].y * h],
                'right_shoulder': [landmarks_list[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x * w, landmarks_list[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y * h],
                'right_hip': [landmarks_list[mp_pose.PoseLandmark.RIGHT_HIP.value].x * w, landmarks_list[mp_pose.PoseLandmark.RIGHT_HIP.value].y * h],
                'right_knee': [landmarks_list[mp_pose.PoseLandmark.RIGHT_KNEE.value].x * w, landmarks_list[mp_pose.PoseLandmark.RIGHT_KNEE.value].y * h],
                'right_ankle': [landmarks_list[mp_pose.PoseLandmark.RIGHT_ANKLE.value].x * w, landmarks_list[mp_pose.PoseLandmark.RIGHT_ANKLE.value].y * h],
                'right_foot_index': [landmarks_list[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX.value].x * w, landmarks_list[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX.value].y * h],
            }
            ...
            # 오류 누적: 내려간 상태('down')일 때만 오류를 기록 (기존 로직 동일)
            if stage == "down":
            ...
            # print(f"랜드마크 처리 또는 각도 계산 오류: {e}")
            pass

        # [개선 2: 스켈레톤 그리기 수정]
        # mp_drawing.draw_landmarks가 레거시 형식을 기대하므로 래퍼 클래스 사용
        if results.pose_landmarks:
            legacy_landmarks_for_drawing = LegacyPoseLandmarks(landmark=results.pose_landmarks[0])
            
            mp_drawing.draw_landmarks(
                image, 
                legacy_landmarks_for_drawing, # 새 래퍼 객체 사용
                mp_pose.POSE_CONNECTIONS,     # 기존 연결선 정보 재사용
